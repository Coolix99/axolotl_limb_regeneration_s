...
class two_morph_euler:
    ...
    def run_simulation(self):
        ...
        for n in tqdm(range(1, self.ndt)):
            ...
            x_grid_prev_gr = x_grid_prev * (1 + self.g * (s_prev) * (f_prev)  * self.dt * growth_region )
            # if n % 1000 == 0:
            #     growth_signal = self.g * (s_prev) * (f_prev) * growth_region
            #     mean_all = np.mean(growth_signal)
            #     growth_signal_gr = growth_signal[growth_region == 1]
            #     mean_gr = np.mean(growth_signal_gr) if growth_signal_gr.size > 0 else 0
            #     max_gr = np.max(growth_signal_gr) if growth_signal_gr.size > 0 else 0

            #     min_bin = np.min(x_grid_prev_gr[2:-2])
            #     max_bin = np.max(x_grid_prev_gr[2:-2])

            #     print(f"Step {n}: mean_all = {mean_all:.4e}, mean_in_growth_region = {mean_gr:.4e}, max_in_growth_region = {max_gr:.4e}, L = {L:.4f}, min_bin = {min_bin:.4e}, max_bin = {max_bin:.4e}")
